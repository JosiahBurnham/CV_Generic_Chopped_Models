...
class VGG_19(): 
    ...
    def __init__(self, wd, num_outputs, l2, pool_rate, output=True, prog_pool = True):
        ...
        self.conv8 = layers.Conv2D(filters=256, kernel_size=(3,3), strides=(1,1), padding='same', activation="relu", name="conv8", trainable=False, kernel_regularizer=keras.regularizers.l2(self.l2))
        ...
        self.conv14 = layers.Conv2D(filters=512, kernel_size=(3,3), strides=(1,1), padding='same', activation="relu", name="conv14", trainable=False, kernel_regularizer=keras.regularizers.l2(self.l2))
        self.conv15 = layers.Conv2D(filters=512, kernel_size=(3,3), strides=(1,1), padding='same', activation="relu", name="conv15", trainable=False, kernel_regularizer=keras.regularizers.l2(self.l2))
        self.conv16 = layers.Conv2D(filters=512, kernel_size=(3,3), strides=(1,1), padding='same', activation="relu", name="conv16", trainable=False, kernel_regularizer=keras.regularizers.l2(self.l2))
    ...
    def get_model(self, layer_level):
        ...
        if(self.prog_pool and layer_level not in [3,6,11,16,21]):
            model.add(layers.MaxPooling2D(pool_size=(ds_size, ds_size), strides=(ds_size,ds_size), padding='valid', data_format="channels_last", name="prog_pool"))
